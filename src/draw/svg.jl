...
function drawline!(canvas::Canvas, uv::SMatrix{2,2}, color::Color, dashed::Bool)
    ...
    uvf = crdf.(uv)
    elem = """<line x1="$(uvf[1, 1])" y1="$(uvf[1, 2])"
     x2="$(uvf[2, 1])" y2="$(uvf[2, 2])" stroke="$(rgbf(color))"$(option)/>
    """
...
function drawwedge!(canvas::Canvas, uv::SMatrix{2,2}, color::Color)
    ...
    uvlen = hypot(utov(uv)...)
    tf = transformmatrix(
        [uvlen, uvlen / 2 * canvas.wedgewidthf],
        utov(uv) / uvlen * [1, -1], uv[1, :]
    )
...
function drawdashedwedge!(canvas::Canvas, uv::SMatrix{2,2}, color::Color)
    ...
    uvlen = hypot(utov(uv)...)
    tf = transformmatrix(
        [uvlen / 7, uvlen / 14 * canvas.wedgewidthf],
        utov(uv) / uvlen * [1, -1], uv[1, :]
    )
    tfarr = reshape(tf[1:2, :], (1, 6))
    tftxt = join(crdf.(tfarr), " ")
...
function drawwave!(canvas::Canvas, uv::SMatrix{2,2}, color::Color)
    uvlen = hypot(utov(uv)...)
    tf = transformmatrix(
        [uvlen / 7, uvlen / 2 * canvas.wavewidthf],
        utov(uv) / uvlen * [1, -1], uv[1, :]
    )
    tfarr = reshape(tf[1:2, :], (1, 6))
    tftxt = join(crdf.(tfarr), " ")
...
function drawtext!(canvas::Canvas, pos::SVector{2}, atom::Atom, align=:center)
    ...
    elem = """<text x="$(argf[1])" y="$(argf[2])" font-size="$(canvas.fontsize)"
